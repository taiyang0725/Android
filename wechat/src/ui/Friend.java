...
public class Friend extends AppActivity implements OnScrollListener, OnRefreshListener{
	...
	private void handleFriends(StrangerEntity entity, int action) {
		...
		if(entity.userList.size() == UIHelper.LISTVIEW_COUNT){//10					
			lvDataState = UIHelper.LISTVIEW_DATA_MORE;//1
			mAdapter.notifyDataSetChanged();
		}
		else {
			lvDataState = UIHelper.LISTVIEW_DATA_FULL;//3
			mAdapter.notifyDataSetChanged();
		}
		if(datas.isEmpty()){
			lvDataState = UIHelper.LISTVIEW_DATA_EMPTY;//4
		}
